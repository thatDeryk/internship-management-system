...
export class CoordinatorProfileComponent implements OnInit {
  ...
  coordinatorProfile = new FormGroup( {
  ...
  constructor ( private coordinatorService: CoordinatorService, private auth: AuthManagerService ) {
  }
  ...
  onSubmit () {
    ...
    this.auth.changePassword( data ).subscribe( result => {
      ...
      this.coordinatorProfile.reset();
      setTimeout( () => {
        this.coordinatorProfile.patchValue( this.userData );
  ...
  prepareData (): object {
    const data = this.coordinatorProfile.value;
    return {
  ...
  ngOnInit () {
    const userData = this.coordinatorService.getLocalStorageData();
    this.userData  = userData;
    this.coordinatorProfile.patchValue( userData );
  }
}
